...
public class TestTransparentTileProvider {
	...
	public void tileAt() {
		...
		TransparentTileProvider transTP = new TransparentTileProvider(isoTileProvider, 0.5);
		Tile tileToProduce = null;
		try {
			tileToProduce = transTP.tileAt(11, 1061, 724);
		} catch (IOException e) {
			e.printStackTrace();
		}
		
		try {
			ImageIO.write(tileToProduce.image(), "png", new File("imageTrans.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
